...
export function ABIMethodCard({ 
  ...
  const getBadgeLabel = () => {
    if (method.type !== 'function' && method.type) return method.type;
    return isReadOnly ? 'read-only' : 'public';
  };
  ...
  const getBadgeStyles = () => {
    ...
    // read-only (view) — green
    if (isReadOnly) return "bg-green-500/10 text-green-500 hover:bg-green-500/20";
    // public (state-changing) — orange
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        {(method.type === 'function' || !method.type) && method.name && (
          <Button
            variant="outline"
            size="sm"
            className={cn(
