...
export function useMonacoEditor({ 
  ...
  const initializeEditor = useCallback(() => {
    ...
    // Initialize Clarity language support
    initializeMonaco();

    // Create or get the model
    const modelUri = monaco.Uri.parse('file:///contract.clar');
    modelRef.current = monaco.editor.getModel(modelUri) ||
                      monaco.editor.createModel(value, 'clarity', modelUri);
  ...
  const updateTheme = useCallback(() => {
    ...
    monaco.editor.setTheme(isDark ? 'clarity-dark' : 'clarity-light');
  }, []);
